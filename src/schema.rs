...
pub fn generate_config_schema() -> Result<Value> {
    ...
    if let Value::Object(ref mut schema_obj) = schema_value {
        schema_obj.insert("$schema".to_string(), json!("http://json-schema.org/draft-07/schema#"));
        schema_obj.insert("title".to_string(), json!("Conclaude Configuration"));
        schema_obj.insert("description".to_string(), json!("Configuration schema for Conclaude - Claude Code hook handler"));
...
pub fn get_schema_url() -> String {
    "https://github.com/conneroisu/conclaude/releases/latest/download/conclaude-schema.json".to_string()
}
...
"#;
    ...
    fn test_generate_yaml_language_server_header() {
        ...
        let custom_header = generate_yaml_language_server_header(Some("https://example.com/schema.json"));
