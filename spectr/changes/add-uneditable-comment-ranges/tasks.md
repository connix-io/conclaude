...
- [ ] 2.2 Create language mapping `HashMap<&str, CommentSyntax>` (use `lazy_static` or `OnceLock`)
  ...
  - [ ] 2.2.3 Add Java mapping (`.java` → `//` and `/* */`)
  - [ ] 2.2.4 Add Python mapping (`.py`, `.pyw` → `#`)
  - [ ] 2.2.5 Add JavaScript/TypeScript mapping (`.js`, `.jsx`, `.ts`, `.tsx`, `.mjs`, `.cjs` → `//` and `/* */`)
  - [ ] 2.2.6 Add Rust mapping (`.rs` → `//` and `/* */`)
  - [ ] 2.2.7 Add Ruby mapping (`.rb`, `.rake`, `.gemspec` → `#` and `=begin`/`=end`)
  - [ ] 2.2.8 Add Shell mapping (`.sh`, `.bash`, `.zsh` → `#`)
  - [ ] 2.2.9 Add Zig mapping (`.zig` → `//`)
  - [ ] 2.2.10 Add Nim mapping (`.nim`, `.nims`, `.nimble` → `#` and `#[` `]#`)
  - [ ] 2.2.11 Add TSX mapping (`.tsx` → `//` and `/* */` and `{/* */}`)
  - [ ] 2.2.12 Add Svelte mapping (`.svelte` → `//`, `/* */`, and `<!-- -->`)
  - [ ] 2.2.13 Add Astro mapping (`.astro` → `//`, `/* */`, and `<!-- -->`)
  - [ ] 2.2.14 Add HTML mapping (`.html`, `.htm` → `<!-- -->`)
  - [ ] 2.2.15 Add Markdown mapping (`.md`, `.markdown`, `.mdx` → `<!-- -->`)
...
- [ ] 6.1 Create end-to-end test files for each supported language in `tests/fixtures/`
  ...
  - [ ] 6.1.3 Create `test_java_uneditable.java` with protected ranges
  - [ ] 6.1.4 Create `test_python_uneditable.py` with protected ranges
  - [ ] 6.1.5 Create `test_javascript_uneditable.js` with protected ranges
  - [ ] 6.1.6 Create `test_rust_uneditable.rs` with protected ranges
  - [ ] 6.1.7 Create `test_ruby_uneditable.rb` with protected ranges
  - [ ] 6.1.8 Create `test_shell_uneditable.sh` with protected ranges
  - [ ] 6.1.9 Create `test_zig_uneditable.zig` with protected ranges
  - [ ] 6.1.10 Create `test_nim_uneditable.nim` with protected ranges
  - [ ] 6.1.11 Create `test_tsx_uneditable.tsx` with protected ranges
  - [ ] 6.1.12 Create `test_svelte_uneditable.svelte` with protected ranges
  - [ ] 6.1.13 Create `test_astro_uneditable.astro` with protected ranges
  - [ ] 6.1.14 Create `test_html_uneditable.html` with protected ranges
  - [ ] 6.1.15 Create `test_md_uneditable.md` with protected ranges
