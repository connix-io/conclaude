...
func TestDefaultConfig(t *testing.T) {
	...
	if config.Rules.PreventRootAdditions != true {
		t.Errorf("Expected PreventRootAdditions to be true, got %v", config.Rules.PreventRootAdditions)
	}

	if config.PreToolUse.PreventGeneratedFileEdits != true {
		t.Errorf("Expected PreventGeneratedFileEdits to be true, got %v", config.PreToolUse.PreventGeneratedFileEdits)
	}
...
`
	...
	if config.Rules.PreventRootAdditions != false {
		t.Errorf("Expected PreventRootAdditions to be false, got %v", config.Rules.PreventRootAdditions)
	}
	...
	for _, tt := range tests {
		t.Run(tt.filePath, func(t *testing.T) {
			...
			if result != tt.expected {
				t.Errorf("IsAutoGeneratedFile(%q) = %v, expected %v", tt.filePath, result, tt.expected)
			}
		})
	}
}
