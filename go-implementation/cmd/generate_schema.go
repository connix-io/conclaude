...
func init() {
	generateSchemaCmd.Flags().StringVarP(&schemaOutput, "output", "o", "conclaude-schema.json", "Output file path for the schema")
	generateSchemaCmd.Flags().BoolVar(&schemaValidate, "validate", false, "Validate the generated schema")
}
...
func runGenerateSchema(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("%s Generating JSON Schema for conclaude configuration...\n", color.New(color.FgBlue).Sprint("🔧"))
